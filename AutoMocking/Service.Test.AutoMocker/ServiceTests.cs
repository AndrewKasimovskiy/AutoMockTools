...
namespace Service.Test.UseAutoMocker
{
    ...
    class ServiceTests
    {
        ...
        public void AddTask_Failed()
        {
            ...
            var service = state.Mocker.CreateInstance<Service>();
		...
		public void SetTaskStatus_Succeed(string cid, TaskStatus taskStatus)
		{
			...
			Assert.IsTrue(result);
			Mock.Get(state.EfContext).Verify(m => m.SaveChanges(), Times.Never());
		}
			...
			bool recvDateChanged = false)
		{
			...
			Smock.Run((context) =>
			{
				...
				var stateS = GetState();
				var service = stateS.Mocker.CreateInstance<Service>();
				result = service.SetTaskStatus(cid, status);

				Mock.Get(stateS.EfContext).Verify(m => m.SaveChanges(), Times.Once);
		...
		private State GetState()
        {
            ...
            result.Mocker = new AutoMocker();

            var efContext = new Mock<IEfContext>( MockBehavior.Strict );

            var dealTasks = GetDealTasks().ToList().ConfigureDbSet();
            result.DealTasksInitialCount = dealTasks.Count();

			Mock.Get(dealTasks).Setup(m => m.Add(It.Is<Item>(item => item.Cid == "00")))
            ...
            efContext.Setup(m => m.DealTasks).Returns(dealTasks);
            efContext.Setup(m => m.DealTasksArch).Returns(dealArchTasks);
            efContext.Setup(m => m.DealTasksCanceled).Returns(dealCanceledTasks);
            efContext.Setup(m => m.SaveChanges()).Returns(() => result.AffectedEntities);


            result.Mocker.Use(efContext);
            result.EfContext = efContext.Object;
